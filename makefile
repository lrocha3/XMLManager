...
TinyXML2_LIB_DIR = $(LIB_DIR)/tinyxml2/dll
TinyXML2_LIB_DLL = TinyXML2
TinyXML2_LIB_INC_DIR = $(LIB_DIR)/tinyxml2/inc

LIBS_INC += -I $(TinyXML2_LIB_INC_DIR)

#Add new libraries to copy dlls to bin path
ALL_LIBS_FILES = $(LIB_DIR)/tinyxml2/dll/$(TinyXML2_LIB_DLL).dll
...
LDLIBS = -L$(TinyXML2_LIB_DIR)
LDLIBS += -l$(TinyXML2_LIB_DLL)
#? == nothing still
...
all: prepare_environment $(BIN_DIR)/$(BIN)
	$(info Copying dlls to bin folder.)
